...
구글 코랩의 수동 API 키 로딩을 python-dotenv를 사용한 자동 로딩으로 개선했습니다.
"""
...
def get_chromadb_config() -> Dict[str, Any]:
    """
    ChromaDB 연결 설정을 반환하는 함수
    
    환경변수 CHROMADB_MODE에 따라 로컬 파일 기반 또는 Docker HTTP 서버 연결을 결정합니다.
    ...
    신입 개발자 팁:
    - 로컬 모드: 개발 시 빠른 테스트용, 파일 기반 저장
    - Docker 모드: 프로덕션 환경, 네트워크를 통한 ChromaDB 서버 연결
    - Docker 모드에서는 data-tier의 ChromaDB 컨테이너와 연결
    """
...
def get_trade_agent_config() -> Dict[str, Any]:
    ...
    이 함수는 무역 정보를 처리하는 AI 에이전트들이 어떻게 동작할지 
    결정하는 모든 설정값들을 제공합니다.
    
    주요 설정 카테고리:
    1. 모델 설정: 어떤 AI 모델을 사용할지, 얼마나 창의적으로 답할지
    2. 검색 설정: 얼마나 많은 문서를 참조할지, 유사도 기준
    3. 필터링 설정: 어떤 국가/카테고리를 지원할지
    ...
    Returns:
        ...
        - model_name: 사용할 OpenAI 모델 ("gpt-4o-mini"는 빠르고 저렴)
        - temperature: AI 응답의 창의성 (0.0=매우 정확, 1.0=매우 창의적)
        - max_context_docs: 한 번에 참조할 최대 문서 수
        - similarity_threshold: 문서 유사도 임계값 (낮을수록 더 많은 문서 포함)
        - supported_countries: 지원하는 국가 목록
        - supported_categories: 지원하는 제품 카테고리 목록
    
    신입 개발자를 위한 팁:
    - 이런 설정을 함수로 분리하는 이유: 코드 여러 곳에서 동일한 설정 사용
    - 하드코딩하지 않고 함수로 관리하면 나중에 수정이 쉬움
    - temperature 값은 AI 성격을 결정하는 중요한 파라미터
    """
    return {
        "collection_name": "trade_info_collection",
