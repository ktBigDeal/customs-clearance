...
def get_query_normalizer(normalizer_type: str = "universal"):
    """
    Query normalizer 팩토리 함수
    
    Args:
        normalizer_type (str): normalizer 타입 ("universal", "law", "trade")
        
    Returns:
        Query normalizer 인스턴스
    """
    try:
        if normalizer_type.lower() == "law":
            return LawQueryNormalizer()
        elif normalizer_type.lower() == "trade":
            return TradeQueryNormalizer()
        else:
            return UniversalQueryNormalizer()
    except Exception as e:
        logger.error(f"Failed to create query normalizer: {e}")
        return None